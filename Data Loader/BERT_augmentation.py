...
class BERT_Augmentation():
    def __init__(self):
        ...
        self.model = transformers.AutoModelForMaskedLM.from_pretrained(self.model_name).to(self.device)  # 모델을 GPU로 이동
        self.tokenizer = transformers.AutoTokenizer.from_pretrained(self.model_name)
        self.unmasker = transformers.pipeline("fill-mask", model=self.model, tokenizer=self.tokenizer, device=0 if torch.cuda.is_available() else -1)  # GPU 사용 설정
        random.seed(42)
